---
title: "Bay Area Bike Rental Operation Research - Analysis of Bicycle Trip and Weather
  Patterns"
...
output <- output %>% head(10) %>% dplyr::rename(Station = var)
knitr::kable(output)
```
...
output_2 <- output_2 %>% head(10) %>% dplyr::rename(Station = var)
knitr::kable(output_2)
```
...
Hmisc::describe(trip_data$duration)  
```

Analysis of numerical variables using the "describe" function from the "Hmisc" R package was done to obtain information about the trip durations. The mean value of trip duration was of trips in the dataset was `r Hmisc::describe(trip_data$duration)[[4]][[5]]` sec, with the five highest durations being `r Hmisc::describe(trip_data$duration)[[7]][[6]]`, `r Hmisc::describe(trip_data$duration)[[7]][[7]]`, `r Hmisc::describe(trip_data$duration)[[7]][[8]]`, `r Hmisc::describe(trip_data$duration)[[7]][[9]]`, and `r Hmisc::describe(trip_data$duration)[[7]][[10]]` seconds long and the five lowest durations all being approximately 60 seconds or less. Here, trips that were less than 2 minutes (120 seconds long) were removed from the data set using the filter function from the "tidyverse" package; about 2,499 observations were removed from the dataset after completing this step.
...
```{r weather_categorical, include=FALSE}
#analyzing categorical data 
output_weather <- freq(weather_df_1)
```

![FrequencyofWeatherEvents](images/weather_categorical-2.png)
![Frequency of Dates](images/weather_categorical-4.png)
...
rush_hours2_tb <- as.data.frame(rush_hours2_tb[1:10]) %>% dplyr::rename(Station = Var1)
knitr::kable(rush_hours2_tb)
```
...
rush_hours3_tb <- as.data.frame(rush_hours3_tb[1:10]) %>% dplyr::rename(Station = Var1)
knitr::kable(rush_hours3_tb)
```
...
The process for determining the most frequent stations on weekends was largely like the previous analysis except data was filtered to remove weekdays instead of weekends. A new data frame called "rush_hours_wkend" was created, and using the "tidyverse" package, weekdays, which are assigned values of 1 (Monday) to 6 (Friday) were filtered out. The 10 most frequent starting stations on weekends were: `r as.data.frame(rush_hours_wkend_tb)$Var1[1:10]` (Table 5).

```{r table_5, echo=FALSE}
rush_hours_wkend_tb <- as.data.frame(rush_hours_wkend_tb)[1:10,] %>% dplyr::rename(Station = Var1)
knitr::kable(rush_hours_wkend_tb)
``` 
...
```{r weather_pre_process, include=FALSE}
#Weather and Trip Correlation Plot

#formating trip start date as.POSIXct format
trip_clean2$start_date <- as.POSIXct(trip_clean2$start_date, format="%m/%d/%y")
...
## Weather and Bike Rental Patterns
...
```{r naina-comments, echo=FALSE}
# All my comments pertaining to the rubric will be added in this chunk so as to not add something in the middle of your code.
# .Rmd completeness - 2/2 Able to knit without any errors
# Packages - 2/2 All necessary packages are explicitly loaded	
# Text Formatting - 1/2 I think you could put your figure and table captions in italics or bold so that they are clearly separate from the main text of your report. Document is appropriately formatted with one level of headers and text.	
# Output Formatting	- 2/2 Code outputs are properly formatted for printing/viewing. Outputs are disabled for appropriate chunks. Your correlation plots look great, not something my group thought to do and it adds a lot of insights to your report.
